...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirmar']) && $_POST['confirmar'] == '1') {
    ...
    $sqlDel = "DELETE FROM avisos_mep WHERE id = ?";
    $stmtDel = $mysqli->prepare($sqlDel);

    if ($stmtDel) {
        $stmtDel->bind_param("i", $idAviso);
        if ($stmtDel->execute()) {
            $stmtDel->close();
            ...
            $errors[] = "No se pudo eliminar el aviso.";
        }
        $stmtDel->close();
    } else {
...
<head>
    ...
    <title>Eliminar Aviso MEP</title>
    ...
    <div class="container form-page-container my-5">
        <div class="card shadow-sm" style="max-width: 600px; margin: 0 auto;">
            <div class="card-body p-4 text-center">
                <div>
                    <h3 class="text-danger">Eliminar Aviso</h3>
                </div>
                <?php if (!empty($errors)): ?>
                ...
                <?php if (!empty($aviso['resumen'])): ?>
                    <p class="text-muted small">
                        <em><?php echo htmlspecialchars($aviso['resumen'] ?? ''); ?></em>
                    </p>
                <?php endif; ?>

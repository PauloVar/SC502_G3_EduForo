...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mb_strlen($telefono) > 8) {
        $errors[] = "El telefono no puede superar los 25 caracteres.";
    }
    ...
    if (mb_strlen($correo) > 100) {
        $errors[] = "El correo no puede superar los 25 caracteres.";
    }

    if (empty($errors)) {
        $sqlUpdate = "UPDATE centros 
                SET nombre = ?, codigo = ?, provincia = ?, canton = ?, nivel = ?, direccion = ?, telefono = ?, correo = ?
                WHERE id = ?";
